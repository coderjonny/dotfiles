...
export class HackerNewsService {
  ...
  private readonly hiringPattern = /ask hn: who is hiring\?.*\d{4}/i;
  private readonly maxConcurrentRequests = 10; // Parallel processing
  private readonly batchSize = 50; // Batch API calls
   ...
   */
  async fetchHiringPosts(months: number = 24): Promise<HiringPost[]> {
    console.log(chalk.blue(`🔍 Searching for hiring posts from the last ${months} months...`));
    ...
    try {
      ...
      console.log(chalk.dim('📡 Fetching whoishiring user submissions...'));
      const userResponse = await axios.get(`${this.baseUrl}/user/whoishiring.json`);
      const submissions = userResponse.data.submitted || [];
      
      // OPTIMIZATION: Pre-filter by timestamp to reduce API calls
      console.log(chalk.dim('🔍 Pre-filtering submissions by date...'));
      ...
      console.log(chalk.green(`✅ Found ${hiringPostIds.length} hiring posts`));
      ...
      console.log(chalk.green(`🎉 Successfully fetched ${allPosts.length} hiring posts/comments`));
      return allPosts;
      
    } catch (error) {
      console.error(chalk.red(`❌ Error fetching hiring posts: ${error}`));
      throw error;
    }
  }
   ...
   */
  private async batchFetchHiringPosts(submissions: number[], cutoffTimestamp: number): Promise<number[]> {
    const hiringPostIds: number[] = [];
    const progressBar = new ProgressBar('🔎 Scanning posts [:bar] :percent :etas', {
      complete: '█',
      incomplete: '░',
      width: 30,
   ...
   */
  private async batchFetchComments(hiringPostIds: number[]): Promise<HiringPost[]> {
    const allPosts: HiringPost[] = [];
    const progressBar = new ProgressBar('📥 Fetching hiring comments [:bar] :percent :etas', {
      complete: '█',
      incomplete: '░',
      width: 30,
   ...
   */
  private async fetchPost(id: number): Promise<HiringPost | null> {
    try {
   ...
   */
  private async fetchHiringComments(postId: number): Promise<HiringPost[]> {
    const comments: HiringPost[] = [];
    
    try {
      const response = await axios.get(`${this.baseUrl}/item/${postId}.json`, {
        timeout: 5000
      });
      const item = response.data;
      ...
      if (item.kids) {
        ...
        const commentPromises = commentIds.map(async (kidId: number) => {
          try {
   ...
   */
  private isHiringPost(post: HiringPost): boolean {
   ...
   */
  async getLatestHiringPost(): Promise<HiringPost | null> {
    try {
      const userResponse = await axios.get(`${this.baseUrl}/user/whoishiring.json`);
      const submissions = userResponse.data.submitted || [];
      ...
      console.error(chalk.red(`❌ Error fetching latest hiring post: ${error}`));
      return null;
    }
  }
}
